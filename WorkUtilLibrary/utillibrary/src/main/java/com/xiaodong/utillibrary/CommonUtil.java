...
public class CommonUtil {
    ...
    /**
     * 电信号段:133/153/180/181/189/177/170；              1  3578   01379
    ...
    public static boolean isMobilePhone(String phoneNum) {
        if (isNotEmpty(phoneNum)) {
            //11位；
            Pattern p = Pattern.compile("^((13[0-9])|(14[5,7])|(15[0,1,2,3,5,6,7,8,9])|(17[0,6,7,8])|(18[0-9]))\\d{8}$");
    ...
    public static String getDiskDownloadDir(Context context) {
        ...
        if (context != null && CommonUtil.isSDCardAvailable()) {
            ...
            //Android/data/package/files/Downloads
    ...
    public static String getApatchDownloadPath(Context context) {
        ...
        if (isNotEmpty(downDir)) {
            ...
            return downDir + File.separator + "apatch" + File.separator + "out.patch";
        }
        return "";
    }
    ...
    public static String getAddpatchDownloadPath(Context context) {
        String downDir = getDiskDownloadDir(context);
        if (isNotEmpty(downDir)) {
            //当没有apatch文件夹时得建文件夹（httpUrlconnection中文件夹必须存在）
            File patchdir = new File(downDir + File.separator + "addpatch");
            if(!patchdir.exists()){
                patchdir.mkdir();
            }
            return downDir + File.separator + "addpatch" + File.separator + "PATCH.patch";
        }
        return "";
    }
    ...
    public static String getAddPatchApkPath(Context context) {
        String downDir = getDiskDownloadDir(context);
        if (isNotEmpty(downDir)) {
            //当没有apk文件夹时得建文件夹（httpUrlconnection中的文件夹必须存在）
            File apkdir = new File(downDir + File.separator + "apk");
            if(!apkdir.exists()){
                apkdir.mkdir();
            }
            return downDir + File.separator + "apk"+ File.separator + "dest.apk" ;
        }
        return "";
    }
...
//        sb.append(lastDays + "天" + lastHours + "小时" + lastminutes + "分" + lastMs + "秒");
    ...
    public static float[] timeFormatFloat(float timeIns) {
        int dayMs = 24 * 3600;
        int hourMs = 3600;
        int minuteMs = 60;
...
//        sb.append(lastDays + "天" + lastHours + "小时" + lastminutes + "分" + lastMs + "秒");
     ...
     * @param timeIns
     */
//    public static void updateTimeLabel(TextView tv, int timeIns, MyEventCallBack callBack) {
//        if (tv == null) {
//            return;
//        }
//        if (timeIns <= 0) {
...
//    }
     ...
     * @param timeIns
     */
//    public static void updateTimeStatus(View view,int timeIns){
...
//        ImageView imageView = (ImageView) view.findViewById(R.id.cp_status);
//        if (tv == null) {
//            return;
//        }
//        if (timeIns <= 0) {
...
//    }
    ...
    public static String jeFormat(Double je){
        DecimalFormat myformat = new DecimalFormat("##,###,###,###,##0.00");
        return myformat.format(je);
    }

//    public static void loginOutTime(Context context){
//        SpUtil.save2SpString(context, AppConstants.INVITE, "");
//        SpUtil.save2SpBoolean(context, AppConstants.ISLOGIN, false);
//        SpUtil.save2SpString(context, AppConstants.TOKEN, "");
//        SpUtil.save2SpString(context, AppConstants.USERNAME, "");
//        LocalBroadcastManager localBroadcastManager = LocalBroadcastManager.getInstance(context);
//        Intent intent = new Intent("notifyout");
//        localBroadcastManager.sendBroadcast(intent);
//    }

    public static boolean createDirWhenNo(File file){
        if(file!=null&&!file.exists()){
            return file.mkdir();
        }
        return false;
    ...
    public static void uninstall(final String packagename, final FragmentActivity context){
        if(!SpUtil.getSpBooleanValueByKey(context,"isUninstallOld",false)) {
            if (hasApp(packagename, context)) {
//                final NoticeDialog noticeDialog = new NoticeDialog();
//                noticeDialog.setContent("检查到你的设备上有老版本应用");
//                noticeDialog.setYesLabel("卸载老版本");
//                noticeDialog.setNoLabel("稍后卸载");
...
//                    }
//                });
//                noticeDialog.show(context.getSupportFragmentManager(),"noticeUninstall");
            }
//        命令行卸载需要root
...
//                }
    ...
    private static boolean hasApp(String packageName,Context context){
        PackageManager pm = context.getPackageManager();
        List<ApplicationInfo> applicationInfos = pm.getInstalledApplications(PackageManager.GET_ACTIVITIES);
        for (ApplicationInfo applicationInfo : applicationInfos){
            LogUtil.d("applicationssss============"+applicationInfo.packageName);
            if (applicationInfo.packageName.equals(packageName)){
                return true;
            }else {
                SpUtil.save2SpBoolean(context,"isUninstallOld",true);
            }
        }
        return false;
    }
    ...
    /**
     * 获取本应用apk路径(data/app/com.xxx.xxx.apk)
     * */
